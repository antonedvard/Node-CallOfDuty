...
export default class Helper {
  protected _BASEURL: string = "https://my.callofduty.com/api/papi-client/";
  protected _LOGINURL: string = "https://profile.callofduty.com/cod/mapp/";
  ...
  protected readonly _USERAGENT: string =
    "a4b471be-4ad2-47e2-ba0e-e1f2aa04bff9";
  ...
  protected getCurrentSeason() {
    ...
    this.sendRequest(url).then(({ lootStream }) => {
      ...
      let season = parseInt(
        lootSeason.split("loot_season_").filter(Boolean)[0]
      );
    ...
    return new Promise((resolve, reject) => {
      ...
      (this.httpI as AxiosInstance)
        ...
        .then((response: AxiosResponse) => {
          ...
          resolve(
            JSON.parse(
              response.data.replace(/^userInfo\(/, "").replace(/\);$/, "")
            )
          );
        })
    ...
    switch (response.status) {
        ...
        switch (apiErrorMessage) {
          case "Not permitted: user not found":
            return {
              ...
              ok: false,
              msg:
                "Not found. Incorrect username or platform? Misconfigured privacy settings?",
            };
      ...
      case 403:
        return {
          ...
          ok: false,
          msg:
            "Forbidden. You may have been IP banned. Try again in a few minutes.",
        };
      case 500:
        return {
          status: 500,
